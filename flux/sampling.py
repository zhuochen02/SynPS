...
def denoise(
    ...
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        ...
        avg_sim = torch.stack(info['attn_similarities']).mean().item()

        if avg_sim > info['pe_threshold_max']:
            info['pe_weight'] = 0.0
        elif avg_sim < info['pe_threshold_min']:
            info['pe_weight'] = 1.0
        else:
            info['pe_weight'] = (info['pe_threshold_max'] - avg_sim) / (info['pe_threshold_max'] - info['pe_threshold_min'])
        print(f"t={t_curr:.4f}, avg_sim={avg_sim:.4f}, pe_weight={info['pe_weight']:.4f}")
